...
from dermatology.scoring import pasi_component, scorable_percentage, salt_component, vasi_component

router = APIRouter(prefix="/api/v1/dermatology/scoring", tags=["dermatology-scoring"])
...
def calculate_pasi_component(req: PASIComponentRequest, _: dict = Depends(require_roles("doctor", "admin"))):
    return {"value": pasi_component(**req.model_dump())}

@router.post("/percentage")
def calculate_percentage(req: PercentageRequest, _: dict = Depends(require_roles("doctor", "admin"))):
    return {"value": scorable_percentage(req.numerator, req.denominator)}

@router.post("/vasi")
def calculate_vasi(req: VASIRequest, _: dict = Depends(require_roles("doctor", "admin"))):
    return {"value": vasi_component(req.depigmented_area_percent)}

@router.post("/salt")
def calculate_salt(req: SALTRequest, _: dict = Depends(require_roles("doctor", "admin"))):
