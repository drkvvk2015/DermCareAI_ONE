...
router = APIRouter(prefix="/api/v1/dermatology/analytics", tags=["dermatology-analytics"])

@router.post("/cohort-summary")
def cohort(records: list[dict[str, object]], user: dict = Depends(require_roles("admin", "auditor", "doctor"))):
