...
def _jpeg_with_contrast() -> bytes:
    ...
    ImageDraw.Draw(image).ellipse((120, 120, 392, 392), fill="black")
    stream = BytesIO()
    image.save(stream, format="JPEG", quality=95)
    return stream.getvalue()


def test_vision_analysis_returns_relative_measurement() -> None:
...
def test_low_information_image_does_not_produce_measurement() -> None:
