...
export const api = {
  ...
  async reviewAI(encounterId: string, reviewId: string, clinicianDecision: 'accepted' | 'overridden' | 'rejected', overrideLabel?: string) {
    return this.clinicalRequest<Record<string, unknown>>(`/encounters/${encodeURIComponent(encounterId)}/ai-reviews/${encodeURIComponent(reviewId)}`, {
      method: 'PATCH',
      body: JSON.stringify({ clinician_decision: clinicianDecision, clinician_override_label: overrideLabel }),
    });
  },
